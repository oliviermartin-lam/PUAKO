function [n_tot,flagStatus] = getDataNumberFromUserSpecification(dataID,id)
inputs = inputParser;
  ...
  obj_name_tmp = [];
    for j=1:n
        ...
        elseif ischar(id_tmp) || isstruct(id_tmp)            
            if isempty(pos_tmp) && any(pos_tmp) || (~isempty(pos_tmp)  && any(pos_tmp ~= (contains({dataID.trs.id},id_tmp) & contains({dataID.imag.id},id_tmp))) )% avoid redundant call
