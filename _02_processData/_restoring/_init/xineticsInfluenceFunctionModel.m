classdef xineticsInfluenceFunctionModel  < handle
    ...
    methods
        ...
        %% Constructor
        function obj = xineticsInfluenceFunctionModel(pitch,varargin)
            inputs = inputParser;
            inputs.addRequired('pitch',@isnumeric);
            inputs.addParameter('influenceCentre',0,@isnumeric);
            inputs.addParameter('mechCoupling',0.11,@isnumeric);
            inputs.parse(pitch,varargin{:});

            
            obj.pitch                  = inputs.Results.pitch;
            obj.influenceCentre = inputs.Results.influenceCentre;   
            obj.mechCoupling    = inputs.Results.mechCoupling;
                                               
            % List of parameters: see KAON 1151
            m = 0.180267421;
            obj.p = [2.24506, 6.28464*m^2,-18.1956*m^4,31.2025*m^6,76.9336,-39.7956,m];
            obj.splineP = obj.mechCoupling;
        end
        ...
        function if2D = getInfluenceFunction(obj,resolution,range)
            
            c = 1/sqrt(log(1/obj.mechCoupling));
            df = 1e-10;
            mx = sqrt(-log(df)*c^2);
            
            x = linspace(-range/2,range/2,resolution);
            [X,Y] = meshgrid(x);
            r = hypot(X,Y);
            % Define the M(r) function
            tmp     = -150*obj.p(7)^(8)*r.^8;
            ...
            re = (abs(r).^e).*obj.p(7).^e;
            % Get the influence function model
            if2D  = exp(-obj.p(5)*re).*(1+obj.p(6)*re).*mask;
        end
        
        function Gdm = getTransferFunction(obj, nActu,resolution,varargin)
            inputs = inputParser;
            inputs.addRequired('nActu',@isnumeric);
            inputs.addRequired('resolution',@isnumeric);
            inputs.addParameter('validActuator',true(nActu),@islogical);
             inputs.addParameter('rebin',2,@isnumeric);
            inputs.parse(nActu,resolution,varargin{:});
            
            validActuator = inputs.Results.validActuator;
            nValid = nnz(validActuator);          
            rebin = inputs.Results.rebin;
            rebin = 2*round(rebin/2);
            
            % Get the transfer function
            dk = (2*nActu-1);
            if2D = obj.getInfluenceFunction(dk,nActu*obj.pitch);
            if2D = puakoTools.enlargePupil(if2D,rebin);
            % Get the Fourier transform
            tf0 = fftshift(fft2(ifftshift(if2D)));
            
            % Get actuators position
            res = getGridCoordinates(nActu,nActu,nActu*obj.pitch);               
            xIF = res.x2D(validActuator(:));
            yIF = res.y2D(validActuator(:));
                                                
            % Sum with shifts            
            [u,v] = freqspace(size(tf0),'meshgrid');                       
            tfdm = zeros(dk*rebin,dk*rebin);
            for k=1:nValid                
                tfdm = tfdm + tf0.*exp(-2i*pi*(u*xIF(k)+v*yIF(k)));
            end                  
           
            Gdm = fftshift(ifft2(ifftshift(tfdm)));
            Gdm = abs(Gdm/Gdm(floor(end/2)+1,floor(end/2)+1));
            if resolution>dk
                Gdm = puakoTools.enlargePupil(Gdm,resolution/dk/2);
            else
                Gdm = puakoTools.interpolateOtf(Gdm,resolution);
            end
                       
        end
        
        function setInfluenceFunction(obj,nActu,resolution,varargin)
            inputs = inputParser;
            inputs.addRequired('nActu',@isnumeric);
            inputs.addRequired('resolution',@isnumeric);
            inputs.addParameter('validActuator',true(nActu),@islogical);
            inputs.addParameter('ratioTelDm',1,@isnumeric);
            inputs.addParameter('offset',[0,0],@isnumeric);
            inputs.addParameter('diameter',[],@isnumeric);
            inputs.parse(nActu,resolution,varargin{:});
                                    
            validActuator = inputs.Results.validActuator;
            ratioTelDm = inputs.Results.ratioTelDm;
            offset = inputs.Results.offset;
            diameter = inputs.Results.diameter;
            
            if isempty(obj.actuatorCoord)
                if obj.influenceCentre==0
                    xIF         = linspace(-1,1,nActu)*(nActu-1)/2*obj.pitch - offset(1);
                    yIF         = linspace(-1,1,nActu)*(nActu-1)/2*obj.pitch - offset(2);
                    [xIF2,yIF2] = ndgrid(xIF,yIF);
                    obj.actuatorCoord = yIF2 + 1i*flip(xIF2);                                        
                    u0 = ratioTelDm.*linspace(-1,1,resolution)*(nActu-1)/2*obj.pitch; % scaled by telescope diamter
                else
                    xIF               = 1:nActu;
                    yIF               = 1:nActu;
                    [xIF2,yIF2]       = ndgrid(xIF,yIF);
                ...
                % Sparse allocation
                modes = spalloc(resolution^2,nValid,nu*nv);
                indIF                   = 1:nActu^2;
                indIF(~validActuator)   = [];
                [iIF,jIF]               = ind2sub([nActu,nActu],indIF);
                kIF                     = 1:nValid;
                ...
                expectedNnz = max(sum(index_u))*max(sum(index_v))*nActu;                                            
                ...
                index       = 1:index(end);
                modes = sparse(s_i(index),s_j(index),s_s(index),resolution^2,nValid);
                obj.modes = modes;
            end                
        end    
    end
