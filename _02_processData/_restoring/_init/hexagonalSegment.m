...
inputs.addParameter('overSamp', max(round(400/size(X0,1)),1),@isnumeric);
...
if  any([0,90,180,270,360,-90,-180,-270,-360] == th)
    ...
    out = puakoTools.rotateIm(out,th);
else
    %1 interpolate the dimension to mitigate pixellization effects
    res = getGridCoordinates(size(Y0,1)*overSamp,size(X0,1)*overSamp,max(abs(X0(:))));    
    ...
    out = puakoTools.interpolate(out,[size(Y0,1),size(X0,1)]);
end