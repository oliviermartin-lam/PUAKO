...
if isfield(trsData,'B')
    ...
    trs.tipTilt.tilt2meter = 3.2*1.268e-05; % !!!! FUDGE FACTOR  estimated from PRIME !
end
...
OBWF = str2num(cell2mat(val(strcmp(list,'OBWF')))) ;
LSPROP = cell2mat(val(strcmp(list,'LSPROP')));
if strcmp(LSPROP,'yes')
    AOFCLGFO = str2num(cell2mat(val(strcmp(list,'AOFCLGFO'))))*1e3; %mm)
    trs.wfs.zstage_defocus = abs(OBWF - AOFCLGFO);
else
    AOFCNGFO = str2num(cell2mat(val(strcmp(list,'AOFCNGFO'))))*1e3; %mm)
    trs.wfs.zstage_defocus = abs(OBWF - AOFCNGFO);
end
trs.wfs.intensity = zeros(400,trs.wfs.nExp);
trs.wfs.intensity(trs.wfs.validSubaperture(:),:) = double(trsData.A.SUBAPINTENSITY);
trs.wfs.wavelength = str2num(cell2mat(val(strcmp(list,'GUIDWAVE')))) *1e-6; %?????
...
trs.mat.Rtt = trs.tipTilt.tilt2meter*permute(MC(:,trs.dm.nCom+1:trs.dm.nCom+2,:),[2,1,3]);
