classdef zernike_puako < handle
    %% Zernike polynomials
    %
    % obj = zernike_puako(j,nPixel) creates a Zernike polynomials object from the
    % modes vector. The radius is set to unity.
    %
    % obj = zernike_puako(...,'resolution',nPixel) creates a Zernike polynomials
    % object from the modes vector and the the number of pixel across the
    % polynomials diameter
    %
    % obj = zernike_puako(...,'radius',r,'angle',o) creates a Zernike polynomials
    % object from the modes vector computed on the given radius and angle.
    % The radius must be normalized to one.
    %
    % obj = zernike_puako(j,nPixel,'pupil',pupilMask) creates a
    ...
    methods
        ...
        %% Constructor
        function obj = zernike_puako(j,resolution,varargin)
            narginchk(1,16);
            p = inputParser;
        ...
        function out = circularCut(obj,delta,largeSmallRadiusRatio)
            ...
            for kj = 1:nj
                out(:,kj) = zernike_puako.fun(obj.j(kj),obj.n(kj),obj.m(kj),rTrunc,oTrunc).*(obj.r(:)<=1).*(rTrunc<=1);
            end
        end
        ...
        function P = smallFootprintExpansion(obj,delta,largeSmallRadiusRatio)
            ...
            function out = integrand(r,o,zi,ni,mi,zj,nj,mj)
                ...
                rTrunc = hypot(xTrunc,yTrunc);
                oTrunc = atan2(yTrunc,xTrunc);
                out = r.*zernike_puako.fun(zi,ni,mi,rTrunc,oTrunc) .* ...
                    zernike_puako.fun(zj,nj,mj,r,o);
            end
    ...
    methods (Static)
        ...
        function P = demoProjection(delta,largeSmallRadiusRatio)
            ...
            zernPlus  = zernike_puako(firstMode:lastMode,32);
            zernMinus = zernike_puako(1:lastMode,32);
