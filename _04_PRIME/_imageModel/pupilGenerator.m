    ...
    methods
        ...
        function mkSpiders(obj,spiderStruct)
            ...
            if (nSpiders~=0) && ~spiderUsrDefined
                ...
                spidersMatrix = logical(puakoTools.interpolate(double(spiders),s));
...
%                     spidersMatrix=~spidersMatrix;
        ...
        function out=zeroPad(obj,ratio,flagReplace)
            ...
            tic
            out=puakoTools.enlargePupil(obj.matrix,ratio);
            if flagReplace
                obj.matrix=out;
        ...
        function out=resize(obj,newNPixels,flagReplace,method)
            ...
            if flagReplace
                obj.matrix=out;
                %obj.segRef.matrix=puakoTools.interpolate(obj.segRef.matrix,round(obj.segRef.nPixels*newNPixels/obj.nPixels));
        ...
        function out=computeModes(obj,type,nModes)
            ...
            switch type
                case 'zernike'
                    zern = zernike_puako(1:nModes,max(sx,sy),'logging',false);
